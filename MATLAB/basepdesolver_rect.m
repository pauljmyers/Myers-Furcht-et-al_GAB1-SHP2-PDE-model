function [sol,r,t_out] = basepdesolver_rect(Co,D,k,R,dr,tf,Nts,dt,tol,maxiters)
% Solver function for the base model in rectangular coordinates. The inputs
% are as follows:
...
for i=2:Nt+1 %loop over time points
    for j=2:Nr %loop over spatial points
        ...
        aSFK(j,2) = (D_Sa*((aSFK(j+1,1)-2.0*aSFK(j,1)+aSFK(j-1,1))/dr^2) - ...
