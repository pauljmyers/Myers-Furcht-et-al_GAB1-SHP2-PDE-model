...
using LinearAlgebra
LinearAlgebra.BLAS.set_num_threads(1)     # if on Apple silicon Mac (M1, etc.), switch this to 1 for best performance
using MKL
...
=#
include("sapdesolver_memb-SFK_MoL.jl") 
...
fn_efast = "eFAST-GSA-res_diffs+kinetic-params_memb-SFKs_$(nruns_ef)-spls-per-param"
...
efast_fig = @pipe gsa_df |> 
    ...
    draw(_, 
        axis=(;height=100, width=300, xticklabelrotation=pi/2, 
            title="eFAST analysis: SFKs active only at cell membrane"
            )
    )
efast_fig |> display
