...
@everywhere begin
    """
        sapdesolver(Co::Vector{Float64}, D::Vector{Float64}, k::Vector{Float64}; 
    ...
    """
    function sapdesolver(Co::Vector{Float64}, D::Vector{Float64}, k::Vector{Float64}; 
        ...
        PG1tot_out = G2PG1[:,end] .+ pGAB1[:,end] .+ PG1Stot_out
    ...
    function pmap_fun_allpars(p; Co=Co, D=Diffs, kvals=kvals, R=R, dr=dr, tf=tf)
        ...
        sol, r_sol = sapdesolver(Co_temp, Diffs_temp, kvals_temp; R=R, dr=dr, tf=tf) # run model
        aSFK = sol.aSFK
        PG1Stot = sol.PG1Stot
    ...
    function pmap_fun_dk(p; Co=Co, D=Diffs, kvals=kvals, R=R, dr=dr, tf=tf, maxiters=100)
        ...
        sol, r_sol = sapdesolver(Co, Diffs_temp, kvals_temp; R=R, dr=dr, tf=tf, tol=1e-3, maxiters=maxiters) # run model
        aSFK = sol.aSFK
        PG1Stot = sol.PG1Stot
    ...
    function pmap_fun_dk_combD(p; Co=Co, D=Diffs, kvals=kvals, R=R, dr=dr, tf=tf)
        ...
        sol, r_sol = sapdesolver(Co, Diffs_temp, kvals_temp; R=R, dr=dr, tf=tf) # run model
        aSFK = sol.aSFK
        PG1Stot = sol.PG1Stot
